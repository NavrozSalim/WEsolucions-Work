        ...
        cancelListingScrape(storeId)
            .then((res) => {
                ...
                onMessage?.(
                    res.data?.message
                        || (stopped
                            ? 'Scrape stopped. Remaining listings stay Pending. Start Scraping will continue with remaining Pending listings.'
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {canScrape && !scrapeBusy && (
                        <Button
                            ...
                            title={withVendor === 0 ? 'No Pending listings to scrape. Use Reset status to re-queue Scraped or Failed rows.' : `Scrape vendor price and stock for ${withVendor} Pending listing(s)`}
                        >
                                    ...
                                    <td className="px-3 py-2.5">
                                        <div className="flex items-center justify-end gap-1">
                                            {canScrape && (
                                                <button
                                                    type="button"
                                                    title="Scrape vendor URL, Nora stock, Vevor feed, or Costway feed"
                                                    ...
                                                    disabled={busy || (!(l.vendor_url || '').trim() && !(l.vendor_id || '').trim() && !/vevor|costway/i.test(l.source_vendor_code || ''))}
                                                >
