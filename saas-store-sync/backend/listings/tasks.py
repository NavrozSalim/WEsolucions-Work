...
def sync_store_orders(*, region: str | None = None, marketplace_codes=None) -> dict:
    ...
    codes = marketplace_codes if marketplace_codes is not None else ["lasoo", "reverb", "mydeal", "etsy", "bunnings"]
...
@shared_task(name="listings.ingest_bulk_upload", bind=True, ignore_result=True, max_retries=0)
def ingest_listing_bulk_upload(self, upload_id, action=""):
